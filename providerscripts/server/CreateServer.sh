...
then
	...
	subnet_id="`/usr/local/bin/linode-cli --text vpcs subnets-list ${vpc_id} | /bin/grep 10.0.1 | /bin/grep adt-subnet | /usr/bin/awk '{print $1}'`"
 	...
 	if ( [ "${snapshot_id}" != "" ] )
	then
		/usr/local/bin/linode-cli linodes create --authorized_keys "${key}" --root_pass "${emergency_password}" --region ${location} --image "private/${snapshot_id}" --type ${server_size} --label "${server_name}" --no-defaults --interfaces '[ { "primary": false, "purpose": "public"},{ "primary": true, "purpose": "vpc", "subnet_id": '"${subnet_id}"', "vpc_id": '"${vpc_id}"' } ]'  
	else
		if ( [ "`/bin/echo ${distribution} | /bin/grep 'Ubuntu 20.04'`" != "" ] )
		then
			/usr/local/bin/linode-cli linodes create --authorized_keys "${key}" --root_pass "${emergency_password}" --region ${location} --image linode/ubuntu20.04 --type ${server_size} --label "${server_name}" --no-defaults --interfaces '[ { "primary": false, "purpose": "public"},{ "primary": true, "purpose": "vpc", "subnet_id": '"${subnet_id}"', "vpc_id": '"${vpc_id}"' } ]' 
		elif ( [ "`/bin/echo ${distribution} | /bin/grep 'Ubuntu 22.04'`" != "" ] )
		then
			/usr/local/bin/linode-cli linodes create --authorized_keys "${key}" --root_pass "${emergency_password}" --region ${location} --image linode/ubuntu22.04 --type ${server_size} --label "${server_name}" --no-defaults --interfaces '[ { "primary": false, "purpose": "public"},{ "primary": true, "purpose": "vpc", "subnet_id": '"${subnet_id}"', "vpc_id": '"${vpc_id}"' } ]'
		elif ( [ "`/bin/echo ${distribution} | /bin/grep 'Ubuntu 24.04'`" != "" ] )
		then
			/usr/local/bin/linode-cli linodes create --authorized_keys "${key}" --root_pass "${emergency_password}" --region ${location} --image linode/ubuntu24.04 --type ${server_size} --label "${server_name}" --no-defaults --interfaces '[ { "primary": false, "purpose": "public"},{ "primary": true, "purpose": "vpc", "subnet_id": '"${subnet_id}"', "vpc_id": '"${vpc_id}"' } ]' 
		elif ( [ "`/bin/echo ${distribution} | /bin/grep 'Debian 10'`" != "" ] )
		then
			/usr/local/bin/linode-cli linodes create --authorized_keys "${key}" --root_pass "${emergency_password}" --region ${location} --image linode/debian10 --type ${server_size} --label "${server_name}" --no-defaults --interfaces '[ { "primary": false, "purpose": "public"},{ "primary": true, "purpose": "vpc", "subnet_id": '"${subnet_id}"', "vpc_id": '"${vpc_id}"' } ]' 
		...
		then
			...
			/usr/local/bin/linode-cli linodes create --authorized_keys "${key}" --root_pass "${emergency_password}" --region ${location} --image linode/debian12 --type ${server_size} --label "${server_name}" --no-defaults --interfaces.primary true --interfaces.purpose vpc --interfaces.subnet_id ${subnet_id} --interfaces.ipv4.vpc ${vpc_id} --interfaces.ipv4.nat_1_1 any		
  		fi
	fi
fi
