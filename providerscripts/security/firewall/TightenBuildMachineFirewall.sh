			...
			/usr/bin/yes | /usr/sbin/ufw enable
                ...
                then
                       ...
                       # /usr/sbin/iptables -A INPUT -p tcp --dport ${buildmachine_ssh_port} -j ACCEPT
                        ...
                        /usr/sbin/iptables -A INPUT  -s `/bin/echo ${ips} | /bin/sed 's/ /,/g'` -p icmp -m state --state RELATED,ESTABLISHED,NEW -m icmp --icmp-type 8 --dport ${buildmachine_ssh_port} -j ACCEPT
