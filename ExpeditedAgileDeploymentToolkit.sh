...
then
        /usr/bin/ssh -p ${SSH_PORT} ${OPTIONS_WS} ${SERVER_USER}@${as_active_ip} "${SUDO} /bin/touch /home/${SERVER_USER}/runtime/GENERATING_SNAPSHOT; ${SUDO} /bin/rm ${HOME}/runtime/INITIAL_BUILD"
        /usr/bin/ssh -p ${SSH_PORT} ${OPTIONS_WS} ${SERVER_USER}@${ws_active_ip} "${SUDO} /bin/touch /home/${SERVER_USER}/runtime/GENERATING_SNAPSHOT; ${SUDO} /bin/rm ${HOME}/runtime/INITIAL_BUILD"
        /usr/bin/ssh -p ${SSH_PORT} ${OPTIONS_WS} ${SERVER_USER}@${db_active_ip} "${SUDO} /bin/touch /home/${SERVER_USER}/runtime/GENERATING_SNAPSHOT; ${SUDO} /bin/rm ${HOME}/runtime/INITIAL_BUILD"
	...
	/usr/bin/find ${BUILD_HOME}/runtimedata/${CLOUDHOST}/${snapshot_build_identifier} -maxdepth 1 -type f ! -name '*.dat' -delete
 	if ( [ -f ${BUILD_HOME}/runtimedata/${CLOUDHOST}/${snapshot_build_identifier}/keys/id_rsa_AGILE_DEPLOYMENT_BUILD_KEY_${BUILD_IDENTIFIER}.pub ] )
  	then
        ...
        /usr/bin/ssh -p ${SSH_PORT} ${OPTIONS_WS} ${SERVER_USER}@${as_active_ip} "${SUDO} /bin/rm /home/${SERVER_USER}/runtime/GENERATING_SNAPSHOT;  ${SUDO} /bin/touch ${HOME}/runtime/SNAPSHOT_GENERATOR"
        /usr/bin/ssh -p ${SSH_PORT} ${OPTIONS_WS} ${SERVER_USER}@${ws_active_ip} "${SUDO} /bin/rm /home/${SERVER_USER}/runtime/GENERATING_SNAPSHOT;  ${SUDO} /bin/touch ${HOME}/runtime/SNAPSHOT_GENERATOR"
        /usr/bin/ssh -p ${SSH_PORT} ${OPTIONS_WS} ${SERVER_USER}@${db_active_ip} "${SUDO} /bin/rm /home/${SERVER_USER}/runtime/GENERATING_SNAPSHOT;  ${SUDO} /bin/touch ${HOME}/runtime/SNAPSHOT_GENERATOR"
